...
ARCH_NAMES = archs_1_FCSA_Fusion_concat.__all__
...
def main():
    ...
    if config['loss'] == 'BCEWithLogitsLoss':
        ...
        criterion = losses.__dict__[config['loss']]().cuda(0)

    cudnn.benchmark = True

    # create model
    model = archs_1_FCSA_Fusion_concat.__dict__[config['arch']](config['num_classes'], config['input_channels'], config['deep_supervision'], embed_dims=config['input_list'], no_kan=config['no_kan'])
    ...
    shutil.copy2('archs_1_FCSA_Fusion_concat.py', f'{output_dir}/{exp_name}/')
